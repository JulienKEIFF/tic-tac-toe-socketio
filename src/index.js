...
io.sockets.on('connection', socket =>{
    socket.on('join', client => {
        ...
        if(canInitGame) {
            const gameController = new GameController(client.room, client.username, socket.id, "", "");
...
function launchGame(socket, first) { io.to(socket).emit('play', first); }

function returnBoard(socket, board){ io.to(socket).emit('board', board); }